...
/* teaching.ts owns the course records; the CV counts them rather than keeping
   a tally of its own. One-way — teaching.ts must never import from here. */
import { courses } from "./teaching";
...
/*
 * Talk counts. Declared ABOVE cvProfile because its stats row is built from
 * them at module-evaluation time, and a const referenced before its
 * declaration is a temporal-dead-zone error rather than a fallback.
 */
...
export const cvProfile = {
  ...
  /*
   * EVERY stat counts the record it summarises. None is typed by hand.
   *
   * Three of the four used to be literals sitting beside a derived publication
   * count, which is the worst of both: a reader cannot tell which numbers are
   * live, and the literals go stale silently. "18 students advised" and "55
   * invited talks" were each a second copy of a fact this same file already
   * held — the advisee arrays, and `talkCounts` — so the moment either list
   * changed, the headline contradicted the list printed below it.
   *
   * A number here should never be a literal. If a new stat has no list to
   * count, add the list first.
   */
  stats: [
    // Refereed journal articles, from the ORCID sync; matches the printed CV.
    { value: String(refereedCount), label: "Publications" },
    { value: String(talkCounts.invited), label: "Invited talks" },
    {
